...
from src.graficas import (
    ...
    grafica_val_mae_multi,
    grafica_val_mape_multi,
    grafica_val_mse_multi,
    grafica_val_r2_multi,
    grafica_val_rmse_multi,
    grafica_velocidad_multi,
)
...
if __name__ == "__main__":
    ...
    for dataset in DATASETS:
        ...
        for H in HORIZONTES:
            ...
            modelo = TreeDRNet(
                ...
                profundidad_arbol=TD_PROF,
                num_ramas=TD_RAMAS,
                hidden_gate=TD_OCULTO,
                hidden_core=TD_OCULTO,
                mlp_depth=2,
                dropout=0.10,
                usar_conv_covariate=True,
            )
            ...
            hist = entrenar_validar(
                ...
                paciencia_sched=PACIENCIA_SCHED,
                min_lr=MIN_LR,
                usar_earlystop=USAR_EARLYSTOP,
                paciencia_es=PACIENCIA_ES,
                min_delta_es=MIN_DELTA_ES,
            )
            best = _safe_load(ckpt, map_location=disp)
            modelo.load_state_dict(best["modelo"])
            ...
            save_epoch_history(
                CARPETA_METRICAS_DSH / f"{dataset}_TreeDRNet_L{LONG_VENTANA}_H{H}_hist.csv", hist
            )
            filas = {
                "dataset": dataset,
                "modelo": "treedrnet",
            ...
            filas_test_ds.append(filas)
            filas_test_global.append(filas)
            historiales[nombre] = hist
            modelos_inst[nombre] = modelo
            print(
                f"[TreeDRNet] Test MSE={mse:.6f} MAE={mae:.6f} RMSE={rmse:.6f} MAPE={mape:.3f}% R2={r2:.4f}"
            )

            # Gráficas por época (aunque ahora solo hay 1 modelo, mantenemos la API multi)
            grafica_val_mse_multi(
                dataset, H, historiales, CARPETA_GRAFICAS_DSH / f"{dataset}_L{LONG_VENTANA}_H{H}_mse.png"
            )
            grafica_val_mae_multi(
                dataset, H, historiales, CARPETA_GRAFICAS_DSH / f"{dataset}_L{LONG_VENTANA}_H{H}_mae.png"
            )
            grafica_val_rmse_multi(
                dataset, H, historiales, CARPETA_GRAFICAS_DSH / f"{dataset}_L{LONG_VENTANA}_H{H}_rmse.png"
            )
            grafica_val_mape_multi(
                dataset, H, historiales, CARPETA_GRAFICAS_DSH / f"{dataset}_L{LONG_VENTANA}_H{H}_mape.png"
            )
            grafica_val_r2_multi(
                dataset, H, historiales, CARPETA_GRAFICAS_DSH / f"{dataset}_L{LONG_VENTANA}_H{H}_r2.png"
            )

            grafica_lr_multi(
                dataset, H, historiales, CARPETA_GRAFICAS_DSH / f"{dataset}_L{LONG_VENTANA}_H{H}_lr.png"
            )
            grafica_velocidad_multi(
                dataset,
                H,
                historiales,
                CARPETA_GRAFICAS_DSH / f"{dataset}_L{LONG_VENTANA}_H{H}_velocidad.png",
            )
    ...
    print(
        "Resultados por dataset en: resultados/<DATASET>/H*/{pesos,metricas,graficas} + metricas/ + graficas/"
    )
