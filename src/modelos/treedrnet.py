...
class DResBlock(nn.Module):
    def __init__(self, dim_in: int, hidden: int, horizonte: int, mlp_depth: int = 2, dropout: float = 0.0):
        ...
        for _ in range(mlp_depth):
            ...
            if dropout > 0: mlp += [nn.Dropout(dropout)]
            d = hidden
...
class GatedBranch(nn.Module):
    def __init__(self, dim_in: int, hidden_gate: int, hidden_core: int, horizonte: int,
                 mlp_depth: int = 2, dropout: float = 0.0):
        super().__init__()
        self.gate = nn.Sequential(
            nn.Linear(dim_in, hidden_gate), nn.ReLU(),
            nn.Linear(hidden_gate, dim_in), nn.Sigmoid()
        )
...
class MultiBranchBlock(nn.Module):
    def __init__(self, dim_in: int, horizonte: int, num_ramas: int,
                 hidden_gate: int, hidden_core: int, mlp_depth: int, dropout: float):
        super().__init__()
    ...
    def forward(self, x_flat: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for br in self.branches:
            bc, fc = br(x_flat)
            bcs.append(bc); fcs.append(fc)
        bc = torch.stack(bcs, dim=0).mean(dim=0)
        fc = torch.stack(fcs, dim=0).mean(dim=0)
        return bc, fc

class TreeDRNet(nn.Module):
    def __init__(self, entrada_dim: int, salida_dim: int, horizonte: int, long_ventana: int,
                 profundidad_arbol: int = 2, num_ramas: int = 2,
                 hidden_gate: int = 128, hidden_core: int = 128,
                 mlp_depth: int = 2, dropout: float = 0.0, usar_conv_covariate: bool = True):
        super().__init__()
        ...
        self.block = MultiBranchBlock(
            dim_in=self.flatten_dim, horizonte=horizonte, num_ramas=num_ramas,
            hidden_gate=hidden_gate, hidden_core=hidden_core, mlp_depth=mlp_depth, dropout=dropout
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for _ in range(self.profundidad):
            fcs = []; nuevos_nodos = []
            for nodo in nodos:
                bc, fc = self.block(nodo)
                fcs.append(fc)
                nuevos_nodos.extend([nodo - bc, nodo - bc])
