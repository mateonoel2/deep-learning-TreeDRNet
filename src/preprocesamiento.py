...
class DatasetVentanasETT(Dataset):
    def __init__(self, df: pd.DataFrame, long_ventana: int, horizonte: int,
                 col_objetivo: str, cols_features: list[str],
                 esc_x: Optional[StandardScaler], esc_y: Optional[StandardScaler]):
        ...
        if esc_x is not None: X = esc_x.transform(X)
        if esc_y is not None: y = esc_y.transform(y)
        self.X = X; self.y = y
...
def _ordenar_por_fecha_si_existe(df: pd.DataFrame) -> pd.DataFrame:
    col_date = None
    for c in df.columns:
        if c.lower() == "date": col_date = c; break
    if col_date is None: return df.reset_index(drop=True)
    df = df.copy(); df[col_date] = pd.to_datetime(df[col_date], errors="coerce")
...
def cargar_ett(dataset: str, long_ventana: int, horizonte: int, col_objetivo: str = "OT"):
    ...
    df = pd.read_csv(ruta_csv); df = _ordenar_por_fecha_si_existe(df)
    ...
    n = len(df); n_train = int(n * 0.70); n_val = int(n * 0.10)
    ...
    ds_train = DatasetVentanasETT(df_train, long_ventana, horizonte, col_objetivo, cols_features, esc_x, esc_y)
...
def crear_loaders(ds_train, ds_val, batch: int, workers: int,
                  persistent_workers: bool = False, prefetch_factor: Optional[int] = None,
                  drop_last_train: bool = False):
    usar_persist = persistent_workers and (workers > 0)
    dl_train = DataLoader(
        ds_train, batch_size=batch, shuffle=True, num_workers=workers, pin_memory=True,
        persistent_workers=usar_persist, prefetch_factor=(prefetch_factor if (workers > 0 and prefetch_factor) else None),
        drop_last=drop_last_train
    )
    dl_val = DataLoader(
        ds_val, batch_size=batch, shuffle=False, num_workers=workers, pin_memory=True,
        persistent_workers=usar_persist, prefetch_factor=(prefetch_factor if (workers > 0 and prefetch_factor) else None),
        drop_last=False
    )
